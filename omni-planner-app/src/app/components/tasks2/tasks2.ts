...
export class Tasks2Component implements OnInit, OnDestroy {
  ...
  initializeSubtaskDefaults(): void {
    ...
    this.newSubtask = {
      ...
      startDate: null,//(this.parentLevel1SubtaskForLevel2?.startDate || this.parentTaskForLevel1Subtask?.startDate || this.parentTaskForSubtask?.startDate) || null,
      endDate: null,//(this.parentLevel1SubtaskForLevel2?.endDate || this.parentTaskForLevel1Subtask?.endDate || this.parentTaskForSubtask?.endDate) || null,
      startTime: '00:00',
      endTime: '00:00',
      estimatedHours: undefined,
      priorityOrder: undefined,
  ...
  getMainTaskStartDate(): string | null {
    ...
    // If we are in "Edit Level 2 Subtask" mode, parentTaskForLevel1Subtask is set.

    if (parentTask && parentTask.startDate) {
      const date = new Date(parentTask.startDate);
  ...
  getMainTaskEndDate(): string | null {
    let parentTask = this.parentTaskForSubtask || this.parentTaskForLevel1Subtask;

    if (parentTask && parentTask.endDate) {
      const date = new Date(parentTask.endDate);
      const year = date.getFullYear();
      const month = String(date.getMonth() + 1).padStart(2, '0');
      const day = String(date.getDate()).padStart(2, '0');
      return `${year}-${month}-${day}`;
